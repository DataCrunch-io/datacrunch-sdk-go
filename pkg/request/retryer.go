...
// Note: This function is moved to avoid import cycles - should be called from the datacrunch package
func WithRetryer(cfg interface{}, retryer Retryer) interface{} {
	if retryer == nil {
		retryer = noOpRetryer{}
	}

	// Use reflection-like interface{} approach to avoid import cycle
	if config, ok := cfg.(interface{ SetRetryer(interface{}) }); ok {
		config.SetRetryer(retryer)
	}
	return cfg
}
...
func isNestedErrorRetryable(parentErr dcerr.Error) bool {
	if parentErr == nil {
		return false
	}
...
func shouldRetryError(origErr error) bool {
	...
	case nil:
		// `dcerr.Error.OrigErr()` can be nil, meaning there was an error but
...
func IsErrorThrottle(err error) bool {
	if aerr, ok := err.(dcerr.Error); ok && aerr != nil {
